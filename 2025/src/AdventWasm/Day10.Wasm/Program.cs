...
foreach (var line in lines)
{
    ...
    if (part == 1) totalPart1 += FindMinPressesPart1(goal, buttons);
    else totalPart2 += FindMinPressesPart2(joltage, buttons);
}

Console.WriteLine(part == 1 ? totalPart1 : totalPart2);

static (int[] goal, List<List<int>> buttons, int[] joltage) ParseLine(string line)
{
...
static int FindMinPressesPart1(int[] goal, List<List<int>> buttons)
{
    ...
    for (int mask = 0; mask < (1 << numButtons); mask++)
    {
        ...
        for (int b = 0; b < numButtons; b++)
        {
            ...
            {
                presses++;
                foreach (var light in buttons[b]) state[light] ^= 1;
            }
        }
        bool matches = true;
        for (int j = 0; j < numLights; j++)
            if (state[j] != goal[j]) { matches = false; break; }
        if (matches && presses < minPresses) minPresses = presses;
    }
    return minPresses;
}

static long FindMinPressesPart2(int[] target, List<List<int>> buttons)
{
    int n = buttons.Count;
    int m = target.Length;
    ...
    for (int col = 0; col < n && row < m; col++)
    {
        ...
        for (int i = row + 1; i < m; i++)
            if (Math.Abs(matrix[i, col]) > Math.Abs(matrix[maxRow, col])) maxRow = i;
        ...
        for (int i = 0; i < m; i++)
        {
            if (i != row && Math.Abs(matrix[i, col]) > 1e-9)
            {
                ...
                for (int j = 0; j <= n; j++) matrix[i, j] -= factor * matrix[row, j];
            }
        }
        row++;
    }
    ...
    for (int j = 0; j < n; j++)
    {
        ...
        for (int i = 0; i < rank; i++)
            if (pivotCol[i] == j) { isBasic = true; break; }
    ...
    if (freeVars.Count == 0)
    {
        ...
        for (int j = 0; j < n; j++)
        {
            ...
            if (val < 0 || Math.Abs(solution[j] - val) > 1e-6) return long.MaxValue;
            total += val;
        }
        return total;
    }
    ...
    for (int f = 0; f < numFree; f++)
    {
        ...
        foreach (int c in buttons[btnIdx]) freeMax[f] = Math.Min(freeMax[f], target[c]);
        if (buttons[btnIdx].Count == 0) freeMax[f] = 0;
        freeMax[f] = Math.Min(freeMax[f], target.Max());
    }

    long best = long.MaxValue;
    SearchFreeVars(0, new int[numFree], freeMax, freeVars, matrix, pivotCol, rank, n, ref best);
    return best;
}

static void SearchFreeVars(int idx, int[] freeVals, int[] freeMax, List<int> freeVars,
                     double[,] matrix, int[] pivotCol, int rank, int n, ref long best)
{
    if (idx == freeVals.Length)
    {
        var solution = new double[n];
        for (int f = 0; f < freeVals.Length; f++) solution[freeVars[f]] = freeVals[f];
        for (int i = rank - 1; i >= 0; i--)
        {
            int col = pivotCol[i];
            double rhs = matrix[i, n];
            for (int j = 0; j < n; j++)
                if (j != col) rhs -= matrix[i, j] * solution[j];
        ...
        {
            long val = (long)Math.Round(solution[j]);
            if (val < 0 || Math.Abs(solution[j] - val) > 0.01) return;
            total += val;
        }
    ...
    long currentSum = 0;
    for (int f = 0; f < idx; f++) currentSum += freeVals[f];
    if (currentSum >= best) return;
    for (int v = 0; v <= freeMax[idx]; v++)
    {
        freeVals[idx] = v;
        SearchFreeVars(idx + 1, freeVals, freeMax, freeVars, matrix, pivotCol, rank, n, ref best);
    }
}
