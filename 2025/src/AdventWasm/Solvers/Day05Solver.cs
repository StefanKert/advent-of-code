...
{
    public string Title => "Range Merging";
    public string Description => "Merge overlapping ranges and validate IDs against merged ranges.";
    ...
    private class Range : IComparable<Range>
    {
        ...
        public int CompareTo(Range? other)
        {
            ...
            if (FromInclusive != other.FromInclusive)
                return FromInclusive.CompareTo(other.FromInclusive);
    ...
    private static bool MergeRanges(List<Range> ranges)
    {
        for (int i = 1; i < ranges.Count; i++)
        {
            ...
            if (ranges[i].FromInclusive <= ranges[i - 1].ToInclusive &&
                ranges[i].ToInclusive <= ranges[i - 1].ToInclusive)
            {
                ranges.RemoveAt(i);
                return true;
            }
    ...
    public string SolvePart1(string input)
    {
        ...
        var count = ids.Count(id =>
            ranges.Any(r => r.FromInclusive <= id && r.ToInclusive >= id));
    ...
    public string SolvePart2(string input)
    {
        ...
        var total = ranges
            .Where(r => r.FromInclusive > 0)
            .Sum(r => r.ToInclusive - r.FromInclusive + 1);
